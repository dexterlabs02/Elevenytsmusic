...
from GraceMusic import config, logger, userbot
...
class MongoDB:
    def __init__(self):
        ...
        if not config.MONGO_URL:
            logger.warning("⚠️ MongoDB connection string not configured. Using DynamoDB only.")
            self.mongo = None
            self.db = None
            return
    ...
    async def get_admins(self, chat_id: int, reload: bool = False) -> list[int]:
        from GraceMusic.helpers._admins import reload_admins
